...
| `RoutingResult`, `SelectedExpert` | `src/dequorum/routing/result.py` | Shared between both router implementations |
| `ExpertAnswer`, `NetworkResponse` | `src/dequorum/inference/pipeline.py` | The query-time output types |
| `CompositionResult` | `src/dequorum/inference/composition.py` | How N expert answers got combined |
...
From [contributor-intake.md](contributor-intake.md), four new tables show up the moment we move past hand-coded keypairs:
...
| `contributor_experts` | M:N — a contributor can publish under many expert personas, and vice versa. | with `contributors` |
...
Every signed entity — `Contribution`, `Vote`, `Document`, future credentials — uses the **same `Signature` primitive** from `core/node.py`. The proof chain shape never changes; it just gets richer ("contribution X extracted from page 47 of document Y by contributor Z" vs today's "contribution X by expert Y").
...
- `docs/architecture/data-model.dbml` (the canonical schema)
