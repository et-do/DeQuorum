...
**Taken together**, Claim 7 is the safety story. The model has no built-in defense against falsehood (adoption $\approx 1.0$ grounded, 0.83 in conflict), but the two governance mechanisms convert that into a defended system: vote-gated retrieval drops false adoption to 0.125, and reputation-weighted voting raises the attacker's cost ~$9\times$. Correctness is something the network *does*, not something the model *has* — and both mechanisms are testable and, at $n=100$, hold.
