...
- **The attribution value measure is only weakly faithful, and judge-sensitive.** At the full 57-pair scale the embedding marginal correlates with judged quality at ρ ≈ 0.04 under a coarse gold-recall judge and ρ ≈ 0.15 under an LLM judge (§8.6) — a real but weak signal, not yet strong enough to set payouts. A ground-truth test (invented facts where the decisive contribution is known) confirms every method trivially recovers it when distractors are *unrelated*; the decisive experiment is the **contested regime**, where the distractor set includes each fact's near-identical false twin — forcing a method to credit the true note over a plausibly-adopted competitor. That test (\`attribution-truth --distractors hard\`) is the pending resolution for the economic layer; a judge-grounded or routing-based estimator is the candidate.
