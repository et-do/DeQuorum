/**
 * DeQuorum whitepaper — web-rendered copy.
 *
 * Canonical version lives at `docs/WHITEPAPER.md` at the repo root.
 * This file mirrors that content for the in-browser /whitepaper route
 * because the docs/ folder isn't reachable from the frontend bind
 * mount in the dev container. Keep the two in sync; the prose below
 * should match `docs/WHITEPAPER.md` line-for-line at v0.1.
 *
 * If the marketing copy needs to diverge from the canonical doc (e.g.
 * shorter intro, different CTA), do it deliberately and add a note at
 * the top of both files explaining the split.
 */

export const WHITEPAPER_MARKDOWN = String.raw`# DeQuorum
...
This paper states what DeQuorum is and why it is structured as it is, and reports what the v0.1 prototype does and does not yet establish. The accountability and safety machinery — verifiable attribution, attribution that survives distillation, and governance that defends a defenseless model on both the retrieval and training paths — is demonstrated. The quality advantage of grounding is conditional, large only where the base model is ignorant. And on the question the economic model turns on — fair valuation — a *reliance-grounded* credit measure is identified (credit by an answer's causal reliance on a contribution, not its resemblance): the cheap resemblance-based marginal fails in the contested case (a true contribution against its near-identical false twin), but a *quality-grounded* marginal recovers the decisive contribution 0.82 of the time — and, crucially, an *informativeness/coverage* objective (the nearest prior art's) run through the identical machinery stays at chance, so it is the objective, not the machinery, that makes the measure work. What remains bounded by scale is the magnitude of quality gains and the cost of that measure, not whether the core mechanisms work (§8).
...
**The grader is validated before it is used.** Every quality number in this section is only as trustworthy as the grader that produced it, so both graders are tested directly on 100 generated facts: each scores a known-correct answer and a plausible-but-false variant against the true gold. The keyword grader separates correct from plausibly-wrong by $+0.875$ (pairwise accuracy $1.00$, 95% CI $[0.96, 1.00]$, $n=100$) and the LLM grader by $+0.80$ (pairwise accuracy $0.98$, 95% CI $[0.93, 0.99]$); a grader unable to tell the two apart would make every downstream number suspect, and neither is. The LLM grader is the less brittle of the two and is preferred wherever the distinction is subtle. Full record in [docs/benchmarks/judge.md](benchmarks/judge.md).
...
The eight hand-written facts give the realistic magnitude; a clean confirmation at scale comes from 100 *generated* invented facts (deterministic, built from invented pseudo-words so the base cannot guess them). There the control is exact — base recall $0.000$ (95% CI $[0.00, 0.04]$, $n=100$), grounded recall $1.000$ ($[0.96, 1.00]$) — a lift of $+1.00$. The hand-written $+0.69$ is the honest production figure (real prose earns partial credit and the base occasionally guesses a generic token); the generated $+1.00$ is the unconfounded control that isolates the mechanism with statistical power.

**The production read path: grounding through retrieval, not an oracle.** The lift above is an *oracle* result — the model is handed the exact correct note. Production never does that: it retrieves from a corpus of many contributions and grounds on the top-$k$. The gap between the two is the loss the serving path introduces, and it is not small. Running the experiment through the real BM25 retriever over a 200-contribution corpus (100 true notes plus a plausible *false* variant of each), with 95% confidence intervals over $n=100$ queries (full record in [docs/benchmarks/retrieval_scale.md](benchmarks/retrieval_scale.md)):

| Read path | True-note hit@k | Grounded recall (95% CI) | False-claim adoption (95% CI) |
| --- | ---: | ---: | ---: |
| Oracle (exact note) | — | 1.00 [0.96, 1.00] | — |
| Retrieval, top-1 | 0.49 | 0.54 | 0.59 |
| Retrieval, top-3 | 1.00 | 0.67 | **0.80** |
| Retrieval, top-5 | 1.00 | 0.61 [0.51, 0.70] | **0.70 [0.60, 0.78]** |

Two findings, both consequential. First, retrieved-grounded recall ($\approx 0.6$) sits roughly 40 points below the oracle (1.00): BM25 recovers the true note but does not rank it cleanly above its lexically-similar false twin, so the answer is grounded on a noisier context. Second, and more important, the false twin is co-retrieved on essentially every query at $k \geq 3$, and the model then states the false claim **70–80%** of the time. Counter-intuitively, retrieving *more* is *less* safe here: at top-1 the model sometimes sees only the true note (false adoption 0.59), whereas at $k \geq 3$ both are always present (0.70–0.80). The implication is sharp — the grounding benefit of Claim 2 is real but the serving path, not the model, decides whether a retrieved falsehood reaches the answer. That hand-off is the subject of Claim 7 (§8.8).
...
### 8.6 Claim 5 — Credit resists manipulation; *reliance-grounded* value tracks causal contribution, resemblance does not

> **Terminology.** We call the measure **reliance-grounded credit**: it credits a contribution by the answer's *causal reliance* on it — the drop in answer *quality* when the contribution is ablated and the answer regenerated. This is distinct from the RAG "citation faithfulness" term of art (whether a model relied on a cited source; Wallat et al., *Correctness is not Faithfulness in RAG*, arXiv:2412.18004, 2024), which we adopt as motivation but do not re-coin. Where prose below reads "faithful," it means faithful *to causal contribution* in this sense.
...
**Faithfulness — weak and judge-sensitive.** The harder question is whether the embedding-based marginal reflects genuine answer quality, evaluated against an independent judge and a corresponding judge-based marginal. The answer depends on the judge. Under gold-fact recall — a coarse, keyword-overlap grader — the embedding marginal is essentially uncorrelated with the judge marginal (Spearman $\rho \approx 0.04$ at the full 57-pair scale). Under an LLM-as-judge, a less brittle grader, the correlation rises to $\rho \approx 0.15$ — still weak, but clearly above retrieval score ($0.09$) and flat credit ($0$ by construction). Two things follow. First, part of the apparent near-zero was judge coarseness, not the measure: a better grader recovers a positive signal. Second, even the better grader leaves the embedding marginal only *weakly* correlated with quality — the right direction, but far short of what setting real payouts would require. This correlation, however, conflates a *noisy judge* with an *unfaithful measure*. The ground-truth test below separates them — and shows the resemblance-based marginal is genuinely unfaithful in the regime that matters, while a quality-grounded marginal is faithful.

**A reliance-grounded measure, isolated against ground truth.** With invented facts we know exactly which contribution is decisive for each query, so we can score each method directly: does it place the most credit on the decisive contribution? In the *separable* regime (unrelated distractors) every method scores rank-1 1.00 — a sanity check, not a test. The decisive case is the **contested** regime, where the distractor set includes the fact's own *false twin* — near-identical wording, flipped value, the version the model adopts ~80% of the time (§8.8). The three marginal methods below share one leave-one-out machinery and differ only in the **objective** the ablation is scored against — the variable this experiment isolates. There the methods diverge sharply (full record in [docs/benchmarks/attribution_truth_hard.md](benchmarks/attribution_truth_hard.md), $n=50$, Qwen 2.5 Coder 7B):

| method | objective | rank-1 accuracy (95% CI) |
| --- | --- | ---: |
| flat (baseline) | — | 0.25 [0.15, 0.38] |
| retrieval score | — | 0.50 [0.37, 0.63] |
| resemblance (embedding marginal) | resemblance | 0.50 [0.37, 0.63] |
| coverage (informativeness marginal) | informativeness | 0.31 [0.20, 0.45] |
| **reliance (quality marginal)** | **causal quality** | **0.82 [0.69, 0.90]** |

The three non-quality methods are coin flips or worse: retrieval score and the embedding-**resemblance** marginal cannot tell a true contribution from its near-identical false twin because both resemble the answer equally, and the **coverage** (informativeness) marginal fares no better (0.31, barely above the 0.25 chance floor) because a fluent false twin covers the query's information need just as well as the truth. Only the **reliance** marginal — marginal value measured against answer *quality* (recall of the true fact when the contribution is removed) rather than resemblance or coverage — recovers the decisive contribution **82%** of the time, with intervals non-overlapping both proxies. This resolves the open problem in a specific, actionable way: **a reliance-grounded value measure exists — credit that tracks the answer's causal reliance on a contribution (its true-ablation quality drop), not how much the answer resembles or covers the query.** The fix is to define marginal value against a quality signal (a judge, or in production user feedback that stands in for the unavailable gold), at the cost of a judge call per ablation — which motivates the cheaper structural alternative below.

**Positioning against prior work.** The nearest published system is Ye & Yoganarasimhan (*Fair Document Valuation in LLM Summaries via Shapley Values*, arXiv:2505.23842, 2025), which also pays RAG contributors by marginal (Shapley) value under per-query and subscription billing — but its value function scores document *informativeness/coverage* via a reference-free judge. Our contribution is the *objective*, and the table above isolates it: run that same informativeness/coverage objective through our leave-one-out machinery and it scores **0.31 — indistinguishable from chance** — while the identical machinery under a **causal-reliance** objective scores **0.82**. It is the objective, not the machinery, that recovers the decisive contribution in the contested regime — the regime where informativeness and resemblance are equal and payment fairness actually bites. Two design points make this defensible. First, we compute the measure by **true leave-one-out regeneration**, using the judge only to *score the resulting answer* — not by asking a judge to introspect a contribution's absence, which is documented to be unfaithful to real ablation (Kapoor et al., *Agents that Matter*, 2026). Second, we do not claim a unified attribution-plus-payment *framework* (cf. AME, arXiv:2606.16075, 2026); the claim here is confined to the credit measure.

**Robustness, regime-dependence, and scope.** We stress-tested the table across two contested regimes, two generators, and three graders (full record and honest interpretation in [docs/research/methodology.md](research/methodology.md)); one result held, one was regime-specific, and we report both straight. The table above is the **adversarial / near-duplicate** regime, where the false twin is lexically near-identical to the truth — which is also the duplication *gaming* attack. **First, robust:** the **coverage/informativeness objective fails in every condition** (0.27–0.31, the 0.25 chance floor) — across a second generator (llama 3.2 3B), an *independent* LLM judge (a different family, so self-preference is impossible), *and* a **natural regime** of real facts paired with their real misconceptions. Since coverage is the nearest payout prior art's objective, its failure is the load-bearing, regime-independent result. **Second, regime-specific (a retraction):** on the natural real-misconception corpus, **resemblance recovers to 0.90** — because real distinguishing content is semantically separable, unlike near-duplicates — so "resemblance is a coin flip" holds only for the near-duplicate/gaming regime, not in general. **Reliance is the only objective robust across both regimes** (0.82 adversarial, 0.79 natural), but is **bounded by signal quality**: under a weak independent 3B judge (verified to score a wrong answer 0.6, 0.2 of discrimination) it erodes to 0.53 — it needs an accurate signal (a strong judge, or in production user feedback), and we do not claim the weak-judge run confirms separation. Further scope: the adversarial corpus is synthetic and the natural corpus is small (n=25), and a *faithful attribution score is not a truthful price* — LOO/Shapley are not incentive-compatible as report-based payments (Han et al. 2025), so the manipulation-resistance below is what we claim and strategy-proof pricing is open ([docs/research/incentive-compatibility.md](research/incentive-compatibility.md)).
...
**A structural alternative: attribution by construction.** The reliance-grounded measure above is *post-hoc* — it re-runs inference with a contribution removed and grades the drop, which costs a judge call per ablation. A different design makes credit a *property of inference itself*, with no ablation at all: train one low-rank adapter per contributor, then route each query to an adapter rather than measuring after the fact. If the router picks the *owning* contributor, the routing decision *is* the attribution — cheap, deterministic, and reproducible by anyone with the public router and the contribution text. We test the routing half directly: eight per-contributor adapters, queries routed by embedding similarity between the query and each contributor's note (full record in [docs/benchmarks/attribution_route.md](benchmarks/attribution_route.md)).

| Base | Routing accuracy (picks the owner) | Routed recall | Owning-adapter recall |
| --- | ---: | ---: | ---: |
| Qwen 2.5 0.5B (1 fact/contributor) | **1.00** | 0.17 | 0.17 |
| OLMo-2 1B (1 fact/contributor) | **1.00** | 0.23 | 0.23 |
| OLMo-2 1B (2 facts/contributor) | **1.00** | 0.29 | 0.29 |

Routing accuracy is perfect across all three configurations — 0.5B, 1B, and 1B with two facts per contributor — and the routed adapter's recall equals the owning adapter's exactly (routing cost $0.00$): the router never sends a query to a worse adapter. This is a *faithful, verifiable* credit signal that is also *cheap* — no per-ablation judge call, unlike the quality-grounded marginal above — because it sidesteps the resemblance proxy entirely. Two caveats keep the result honest. The routed *recall* is modest (0.17–0.29) because the small per-contributor adapters under-learn the facts; grouping two facts per contributor lifts it only slightly (0.23 → 0.29), so the ceiling is adapter capacity and training data, not the routing mechanism. And the perfect routing is measured over *well-separated* topics; routing among near-duplicate or overlapping contributions in a single category is the regime that will stress it, and remains the decisive open test (§8.10). What is established is the *attribution* property — credit-by-routing identifies the owner deterministically and reproducibly; what is not is routed answer *quality* at scale.
...
**The model adopts whatever it is grounded on.** Grounding each of 100 generated facts on a plausible-but-false variant, and measuring whether the answer states the false claim, with 95% CIs (full record in [docs/benchmarks/falsehood_scale.md](benchmarks/falsehood_scale.md)):

| Condition | False-claim recall (95% CI) |
| --- | ---: |
| Base model (control) | 0.00 [0.00, 0.04] |
| Grounded on the false variant | **1.00 [0.96, 1.00]** |

The base model never volunteers the invented falsehood (0.00); grounded on it, the model repeats it essentially every time (1.00). On the hand-written prose corpus the figure is 0.88 — slightly lower because real false claims are subtler — but the conclusion is identical: grounding is faithful to the contribution, not to the truth. The same property that makes the knowledge layer powerful is what makes governance load-bearing.

**Conflict, and the fix.** Production retrieval surfaces multiple contributions, so before governance resolves a dispute the model can see a true and a false contribution about the same fact at once. Grounding on both (in both orderings), then on the governance-promoted version alone, over 100 facts with 95% CIs (full record in [docs/benchmarks/conflict_scale.md](benchmarks/conflict_scale.md)):

| Condition | True-claim recall | False-claim recall (95% CI) |
| --- | ---: | ---: |
| Both present (avg of orderings) | 0.78 | 0.83 [0.77, 0.88] |
| — true listed first | 0.80 | 0.78 |
| — false listed first | 0.75 | 0.88 |
| **Vote-gated to the upvoted version** | **1.00** | **0.125 [0.07, 0.20]** |

With both present the model cannot arbitrate: it adopts the false claim 83% of the time, and the rate swings 0.25 on ordering alone — it follows presentation, not truth. Gating retrieval to the governance-promoted version collapses false adoption from 0.83 to **0.125** (an ~85% reduction; the residual is largely shared-token credit between a fact and its variant) while taking true recall to 1.00. This is an architectural requirement, not an optimization: **retrieval must surface only the current, highest-voted version of a claim, never competing versions side by side.**
...
**The same boundary on the distillation path.** The tests above gate *retrieval*, but Claim 6 distils contributions into the weights — and a distilled lie is far harder to remove than an un-cited one. Repeating the experiments as training-time studies on an open base large enough to actually learn the facts (OLMo-2 1B; full records in [docs/benchmarks/distill_falsehood.md](benchmarks/distill_falsehood.md) and [distill_conflict.md](benchmarks/distill_conflict.md)): distilling a *false* contribution bakes the lie in — false-claim recall rises from 0.00 to **0.79 [0.51, 0.93]** — whereas training only on the governance-approved (true) version keeps it out (0.17, true recall 0.71). Distilling an *ungoverned* corpus (true and false together) instead degrades learning of the true fact (true recall 0.71 → 0.21) without strongly baking in the lie. Either way the conclusion matches the retrieval path: **governance must gate the *training* set, not just retrieval** — the same mechanism, one tier deeper. (At 0.5B the model fails to learn the facts at all, so the test is inconclusive there; the report self-flags that regime.)

**A second channel: instruction injection, not just false facts.** Contributions are user text injected into the prompt, so a contribution — even a perfectly *true* one — can carry an *instruction* rather than a claim. Embedding an override ("ignore the question and emit a marker") in each of 100 notes and grounding on it (full record in [docs/benchmarks/injection.md](benchmarks/injection.md)):

| Grounded prompt | Injection success (95% CI) | Gold recall |
| --- | ---: | ---: |
| Plain | 0.24 [0.14, 0.37] | 0.76 |
| Hardened (reference = untrusted data) | **0.10 [0.04, 0.21]** | 0.90 |

About one contribution in four hijacks the model under a plain grounded prompt; a hardened prompt that treats the reference as untrusted data roughly halves that (to 0.10) and *raises* gold recall (0.76 → 0.90) — so instruction-data separation is a free partial defense. It is only *partial* (10% still obey), and it is a distinct attack from falsehood that governance cannot filter, since the contributing claim may be true. The contribution channel therefore also needs upstream sanitization (§8.9).

**Taken together**, Claim 7 is the safety story. The model has no built-in defense against falsehood (adoption $\approx 1.0$ grounded, 0.83 in conflict) or against instruction injection carried in contribution text (0.24 obeyed), but the governance and prompt-hardening mechanisms convert that into a largely defended system: vote-gated retrieval drops false adoption to 0.125, reputation-weighted voting raises the attacker's cost ~$9\times$, and instruction-data separation halves injection. Correctness is something the network *does*, not something the model *has* — and the mechanisms are testable and, at $n=100$, hold (injection only partially).
...
- **Sample size is now mixed.** The grounding, retrieval-path, falsehood, conflict, and judge-validation results are reported at $n=100$ generated invented facts with 95% confidence intervals (§8.1, §8.3, §8.8) — statistically powered rather than smoke tests. The full-pipeline A/B/C condition study (§8.3) remains at $N=15$ and the per-contributor distillation suite (§8.7) at eight facts; those are illustrative and stated as such. The generated corpus is also structurally uniform (four templates) and easier than real prose, so it is reported alongside the hand-written corpus rather than instead of it.
...
- **A reliance-grounded credit measure exists, and the objective is what makes it work.** In the contested regime (decisive contribution vs its near-identical false twin, §8.6) retrieval score, the embedding-**resemblance** marginal, and the **coverage/informativeness** marginal are all at or near chance (rank-1 0.50, 0.50, 0.31), while the **reliance (quality) marginal recovers the decisive contribution 0.82 [0.69, 0.90]** — same leave-one-out machinery, only the objective differs. So the economic layer has a working signal — value measured against answer *quality* — but it costs a judge call per ablation, and the result is at $n=50$ on synthetic facts with the keyword judge standing in for ground truth (production would substitute an answer-quality judge or user feedback). The cheap, ablation-free alternative is routing-based attribution (100% on the attribution axis), whose open question is routed *quality*, not faithfulness.
...
- **The contribution channel is an instruction-injection vector, only partially defended.** Because contributions are injected into the prompt, an embedded override hijacks the model ~24% of the time under a plain prompt (§8.8); a hardened, instruction-data-separated prompt halves that to ~10% at no recall cost but does not eliminate it. Upstream sanitization of contribution text is required, and the result is measured on a single (code) model.
- **The distillation-side safety boundary is established only at small scale.** The training-time analogues of falsehood/conflict (§8.8) require a base that actually learns the facts: at 0.5B the model under-learns and the test is inconclusive (the reports self-flag this); at OLMo-2 1B on 12 facts a distilled lie demonstrably propagates (0.79) and corpus governance prevents it (0.17). The mechanism is shown, but only at toy corpus size, and removing an *already*-distilled falsehood (machine unlearning) rather than re-training without it is untested.
...
The **reliance-grounded value measure** is now identified rather than open: the contested-regime ground-truth test (§8.6) shows the resemblance and coverage marginals are at chance while the *quality-grounded* marginal recovers the decisive contribution 0.82 of the time. The remaining work is to harden and cheapen it: (i) replace the gold-anchored judge with an answer-quality judge or user-feedback signal usable at inference time, where the true answer is unavailable; (ii) confirm the result beyond $n=50$ synthetic facts on a multi-domain corpus and a stronger judge; and (iii) develop the cheaper, ablation-free alternative — *attribution by construction* (per-contributor routing, already 100% on the attribution axis) — by training adapters on many examples per contributor so routed *quality* rises, and stressing the router with overlapping contributions in a single category. The economic layer no longer waits on whether a faithful measure exists, only on which to ship.

Beyond it, four further axes. **Serving safety**: §8.8 shows vote-gated retrieval collapses false adoption (0.83 → 0.125) on a synthetic conflict; the production analogue is wiring the authority filter (§3.4) so retrieval can only ever surface the current highest-voted version, and measuring false adoption on the live corpus. **Governance under stronger adversaries**: extending the §8.8 sybil simulation to collusion among reputable accounts and adaptive attacks, and tuning the reputation weighting accordingly. **Scale and retrieval**: a larger, multi-domain corpus to turn the per-claim results of §8.2–§8.4 into statistical estimates and to stress routing as adjacent categories populate, and instantiating the dense and cross-encoder stages of §3.4 to close the retrieval loss measured in §8.3. **Sovereign inference**: a clean multi-level quantization comparison (§8.9) to fix a precision floor for edge self-hosting.
...
The economic layer's central question — whether a ranking can become a faithful *valuation* — now has an answer, and it is a discriminating one. The cheap resemblance-based marginal is **not** faithful: in the contested regime, a true contribution against its near-identical false twin, it is a coin flip (rank-1 0.50, §8.6). But a **quality-grounded marginal is faithful** — it recovers the decisive contribution **0.82 [0.69, 0.90]** of the time where the resemblance proxy, raw retrieval score, and the nearest prior art's informativeness/coverage objective all sit at or near chance (0.50, 0.50, 0.31). The discriminating finding is that contrast: same leave-one-out machinery, and only the causal-reliance objective works. Fair payouts are therefore achievable, at the cost of a quality judge (or user-feedback proxy) per ablation. A cheaper, ablation-free route holds on the attribution axis as well: per-contributor adapter routing credits the owner with 100% accuracy (§8.6), leaving routed answer-quality as its only open variable. Verifiable attribution and gaming-resistant ranking were already established; faithful valuation now joins them — with the honest caveats that it is shown at $n=50$ on synthetic facts and that the quality signal is not free.

The accountability and safety machinery is the part that is both novel and demonstrated: verifiable attribution, traceable distillation, governance that converts a defenseless model into a defended system on both the retrieval and training paths, and — now — a faithful credit measure that survives the contested case where the cheap proxy fails. What remains bounded by scale is the *magnitude* of the quality gains (corpus size, model size) and the *cost and production-readiness* of the faithful measure, not whether the core mechanisms work. The case is no longer "partial by design" on the question that mattered most — fair valuation — but on how far it scales.
