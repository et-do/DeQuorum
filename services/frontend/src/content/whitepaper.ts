...
**A reliance-grounded measure, isolated against ground truth.** With invented facts we know exactly which contribution is decisive for each query, so we can score each method directly: does it place the most credit on the decisive contribution? In the *separable* regime (unrelated distractors) every method scores rank-1 1.00 — a sanity check, not a test. The decisive case is the **contested** regime, where the distractor set includes the fact's own *false twin* — near-identical wording, flipped value, the version the model adopts ~80% of the time (§8.8). There the methods diverge sharply (full record in [docs/benchmarks/attribution_truth_hard.md](benchmarks/attribution_truth_hard.md), $n=40$):

| method | rank-1 accuracy (95% CI) |
| --- | ---: |
| flat (baseline) | 0.25 [0.14, 0.40] |
| retrieval score | 0.50 [0.35, 0.65] |
| embedding-marginal (resemblance) | 0.50 [0.35, 0.65] |
| **judge-grounded marginal (quality)** | **0.89 [0.75, 0.95]** |

Retrieval score and the embedding-resemblance marginal are coin flips — they cannot tell a true contribution from its near-identical false twin, because both resemble the answer equally. The **judge-grounded marginal** — marginal value measured against answer *quality* (recall of the true fact when the contribution is removed) rather than resemblance to the contribution — recovers the decisive contribution **89%** of the time, decisively above both chance and the resemblance proxies, with non-overlapping intervals. This resolves the open problem in a specific, actionable way: **a reliance-grounded value measure exists — credit that tracks the answer's causal reliance on a contribution (its true-ablation quality drop), not how much the answer resembles it.** The same resemblance proxy that was weak at the correlation scale fails here for the same reason; the fix — already flagged as future work — is to define marginal value against a quality signal (a judge, or in production user feedback that stands in for the unavailable gold). The cost is a judge call per ablation, which motivates the cheaper structural alternative below.

**Positioning against prior work.** The nearest published system is Ye & Yoganarasimhan (*Fair Document Valuation in LLM Summaries via Shapley Values*, arXiv:2505.23842, 2025), which also pays RAG contributors by marginal (Shapley) value under per-query and subscription billing — but its value function scores document *informativeness/coverage* via a reference-free judge. Our contribution is the *objective*: crediting by **causal reliance** (the true-ablation quality drop), which is what distinguishes a true contribution from its near-identical false twin — the regime where informativeness and resemblance are equal and payment fairness actually bites. Two design points make this defensible. First, we compute the measure by **true leave-one-out regeneration**, using the judge only to *score the resulting answer* — not by asking a judge to introspect a contribution's absence, which is documented to be unfaithful to real ablation (Kapoor et al., *Agents that Matter*, 2026). Second, we do not claim a unified attribution-plus-payment *framework* (cf. AME, arXiv:2606.16075, 2026); the claim here is confined to the credit measure.
...
- **A faithful credit measure exists, but the cheap resemblance proxy is not it.** In the contested regime (decisive contribution vs its near-identical false twin, §8.6) retrieval score and the embedding-resemblance marginal are coin flips (rank-1 0.50), while the **judge-grounded marginal is faithful (0.89 [0.75, 0.95])**. So the economic layer has a working signal — value measured against answer *quality* — but it costs a judge call per ablation, and the result is at $n=40$ on synthetic facts with the keyword judge standing in for ground truth (production would substitute an answer-quality judge or user feedback). The cheap, ablation-free alternative is routing-based attribution (100% on the attribution axis), whose open question is routed *quality*, not faithfulness.
...
The **faithful value measure** is now identified rather than open: the contested-regime ground-truth test (§8.6) shows the resemblance-based marginal is a coin flip while the *quality-grounded* marginal recovers the decisive contribution 0.89 of the time. The remaining work is to harden and cheapen it: (i) replace the gold-anchored judge with an answer-quality judge or user-feedback signal usable at inference time, where the true answer is unavailable; (ii) confirm the result beyond $n=40$ synthetic facts on a multi-domain corpus and a stronger judge; and (iii) develop the cheaper, ablation-free alternative — *attribution by construction* (per-contributor routing, already 100% on the attribution axis) — by training adapters on many examples per contributor so routed *quality* rises, and stressing the router with overlapping contributions in a single category. The economic layer no longer waits on whether a faithful measure exists, only on which to ship.
