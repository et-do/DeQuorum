...
**Faithfulness — a negative result at scale.** The harder question is whether the embedding-based marginal reflects genuine answer quality, evaluated against an independent judge (gold-fact recall) and a corresponding judge-based marginal. Across samples of 14, 23, and 57 judge-scored pairs the correlation between the embedding marginal and the judge marginal is $\rho \approx 0.00$, $0.21$, and — at the full scale — $0.04$: essentially no relationship, with the larger sample collapsing the apparent mid-sample signal. Retrieval score is consistently non-positive against judged value ($\rho \approx -0.56$, $-0.20$, $-0.24$). We therefore report a negative finding: **the embedding-resemblance marginal, as defined, is not a faithful estimator of a contribution's effect on answer quality**, and is not suitable to drive payouts. What the measure does provide — manipulation resistance and a per-contribution ranking within an answer — holds, but the link to *quality* does not. Establishing a faithful measure (a judge-grounded estimator, the Shapley variant evaluated against a stronger judge, or a redefinition of marginal value) is the open problem this section surfaces rather than resolves (§8.8).
...
- **The attribution value measure is not faithful as defined.** At the full gold-annotated scale (57 pairs) the embedding marginal is essentially uncorrelated with judged quality (ρ ≈ 0.04; §8.6). It cannot drive payouts in its current form; a faithful estimator is the central open problem for the economic layer.
...
The economic layer carries one unresolved result that scale has now made concrete: the cheap value measure is **not** faithful. At the full benchmark scale the embedding-based marginal value is uncorrelated with judged answer quality (§8.6), so it cannot, as defined, set fair payouts. Verifiable attribution and gaming-resistant *ranking* hold; converting a ranking into a faithful *valuation* is the central problem the economic model must still solve.
