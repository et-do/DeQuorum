...
export default function App() {
  ...
  useEffect(() => {
    fetch("/api/healthz")
      ...
      .then((text) => setHealth({ status: text === "ok" ? "ok" : "error", detail: text }))
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto px-4 py-8 w-full">
        <h2 className="text-2xl font-semibold text-slate-900">Welcome to DeQuorum</h2>
        <p className="mt-2 text-slate-600 max-w-2xl">
          The crowdsourced AI network where every answer is signed by the contributors
          whose knowledge shaped it. This is the v0.1 frontend stub — the real query
          UI, signup flow, and review queue land in upcoming phases.
        </p>
